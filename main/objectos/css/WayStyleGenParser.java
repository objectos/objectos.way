...
class WayStyleGenParser extends WayStyleGenScanner {
  ...
  private Rule rule0(String value) {
    ...
    return switch (prefix) {
      case "h" -> height(value.substring(dashIndex + 1));

      case "m" -> margin(value.substring(dashIndex + 1), Utility.MARGIN);
      case "mx" -> margin(value.substring(dashIndex + 1), Utility.MARGIN_X);
      case "my" -> margin(value.substring(dashIndex + 1), Utility.MARGIN_Y);
      case "mt" -> margin(value.substring(dashIndex + 1), Utility.MARGIN_TOP);
      case "mr" -> margin(value.substring(dashIndex + 1), Utility.MARGIN_RIGHT);
      case "mb" -> margin(value.substring(dashIndex + 1), Utility.MARGIN_BOTTOM);
      case "ml" -> margin(value.substring(dashIndex + 1), Utility.MARGIN_LEFT);

      default -> rule1(value, dashIndex);
    };
  }
  ...
  private Rule margin(String value, Utility kind) {
    return switch (value) {
      case "auto" -> kind.nameValue(variants, name, "auto");
      default -> spacing(name, value, kind);
    };
  }
  ...
  private static Rule rule1(String className, int dashIndex) {
    return Rule.NOOP;
  }
