...
final class HttpRequestMatcherX implements Predicate<HttpRequest> {
  ...
  record Segment(SegmentKind kind, String value, char c) {
  ...
  private HttpRequestMatcherX(Kind kind, Object state) {
    this.kind = kind;
  ...
  public static HttpRequestMatcherX parsePath(String pathExpression) {
    return parse(pathExpression, false);
  }

  public static HttpRequestMatcherX parseSubpath(String pathExpression) {
    return parse(pathExpression, true);
  }

  private static HttpRequestMatcherX parse(String pathExpression, boolean subpath) {
    Parser parser;
  ...
  static HttpRequestMatcherX methodAllowed(HttpMethod value) {
    return new HttpRequestMatcherX(Kind.METHOD_ALLOWED, value);
  }

  static HttpRequestMatcherX methodNotAllowed(Set<HttpMethod> allowedMethods) {
    return new HttpRequestMatcherX(Kind.METHOD_NOT_ALLOWED, allowedMethods);
  }

  static HttpRequestMatcherX pathExact(final String value) {
    return new HttpRequestMatcherX(Kind.PATH_EXACT, value);
  }

  static HttpRequestMatcherX pathSegments(List<Segment> segments) {
    ...
    return new HttpRequestMatcherX(Kind.PATH_SEGMENTS, segments);
  }
  ...
  static HttpRequestMatcherX subpathExact(String value) {
    return new HttpRequestMatcherX(Kind.SUBPATH_EXACT, value);
  }

  static HttpRequestMatcherX subpathSegments(List<Segment> segments) {
    final Set<String> distinct;
    distinct = Util.createSet();
    ...
    return new HttpRequestMatcherX(Kind.SUBPATH_SEGMENTS, segments);
  }
  ...
  public final boolean equals(Object obj) {
    return obj instanceof HttpRequestMatcherX that
        && kind == that.kind
  ...
  public final boolean test(HttpRequest request) {
    ...
    return switch (kind) {
      case METHOD_ALLOWED -> {
        ...
        yield method == state || (method == HttpMethod.HEAD && state == HttpMethod.GET);
      }
      ...
      case METHOD_NOT_ALLOWED -> {
        ...
        yield !(allowed.contains(method) || (method == HttpMethod.HEAD && allowed.contains(HttpMethod.GET)));
      }
  ...
  final HttpRequestMatcherX with(HttpPathParam[] conditions) {
    ...
    return new HttpRequestMatcherX(newKind, state);
  }
