...
final class HttpExchangeImpl implements HttpExchange, Runnable, Closeable {
  ...
  private record Notes(
      ...
      Note.Long1Ref1<HttpExchange> readEof,
      Note.Long1Ref1<HttpExchange> readMaxBuffer,
      Note.Long1Ref2<HttpExchange, IOException> readIOException,

      Note.Long1Ref2<Path, HttpExchange> bodyFile,

      Note.Long1Ref1<IOException> writeIOException,

      Note.Long1Ref2<ClientError, HttpExchange> badRequest,
      Note.Long1Ref1<HttpExchange> lengthRequired,
      Note.Long1Ref1<HttpExchange> contentTooLarge,
      Note.Long1Ref1<HttpExchange> uriTooLong,
      Note.Long1Ref1<HttpExchange> requestHeaderFieldsTooLarge,
      Note.Long1Ref2<HttpExchange, Throwable> internalServerError,
      Note.Long1Ref1<HttpExchange> notImplemented,
      Note.Long1Ref1<HttpExchange> httpVersionNotSupported,
  ...
  public final void handle(HttpHandler handler) {
    try {
      handler.handle(this);
    } catch (Http.AbstractHandlerException ex) {
      ex.handle(this);
  ...
  private byte executeBadRequest() {
    ...
    statusUnchecked(HttpStatus.BAD_REQUEST);

    headerUnchecked(HttpHeaderName.DATE, now());

    headerUnchecked(HttpHeaderName.CONTENT_TYPE, "text/plain; charset=utf-8");

    headerUnchecked(HttpHeaderName.CONTENT_LENGTH, message.length);

    headerUnchecked(HttpHeaderName.CONNECTION, "close");
  ...
  private byte executeLengthRequired() {
    ...
    statusUnchecked(HttpStatus.LENGTH_REQUIRED);
  ...
  private byte executeContentTooLarge() {
    ...
    statusUnchecked(HttpStatus.CONTENT_TOO_LARGE);
  ...
  private byte executeUriTooLong() {
    ...
    statusUnchecked(HttpStatus.URI_TOO_LONG);

    headerUnchecked(HttpHeaderName.DATE, now());

    headerUnchecked(HttpHeaderName.CONTENT_LENGTH, "0");
  ...
  private byte executeRequestHeaderFieldsTooLarge() {
    ...
    statusUnchecked(HttpStatus.REQUEST_HEADER_FIELDS_TOO_LARGE);
  ...
  private byte executeNotImplemented() {
    ...
    statusUnchecked(HttpStatus.NOT_IMPLEMENTED);
  ...
  private byte executeHttpVersionNotSupported() {
    ...
    statusUnchecked(HttpStatus.HTTP_VERSION_NOT_SUPPORTED);
  ...
  final byte internalServerError(Throwable ise) {
    ...
    statusUnchecked(HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  private void headerUnchecked(HttpHeaderName name, long value) {
  ...
  private void headerUnchecked(HttpHeaderName name, String value) {
    ...
    if (name == HttpHeaderName.CONNECTION && value.equalsIgnoreCase("close")) {
      bitClear(BIT_KEEP_ALIVE);
    }

    else if (name == HttpHeaderName.CONTENT_LENGTH) {
      bitSet(BIT_CONTENT_LENGTH);
    }

    else if (name == HttpHeaderName.TRANSFER_ENCODING) {
  ...
  private void note(Note.Long1Ref1<HttpExchange> note) {
