...
public class LoginTest {
  ...
  public void testCase01() throws IOException {
    ...
    assertEquals(
        ...
        <form class="my-auto w-full bg-white sm:w-auto" id="login-form" action="/login" method="post"><input name="step" type="hidden" value="one">
  ...
  public void testCase02() {
    ...
    assertEquals(
        ...
        \r
        589\r
        [{"cmd":"html","value":"<!DOCTYPE html><html><head><title>Login Page</title></head><body class='bg-gray-100' data-frame='root' data-frame-value='login'><div class='mx-4 flex h-screen flex-col items-center sm:mx-auto'><form class='my-auto w-full bg-white sm:w-auto' id='login-form' action='/login' method='post'><input name='step' type='hidden' value='two'><input name='login' type='hidden' value='admin'><div class='mb-1 p-4 pb-0 text-2xl leading-none tracking-tighter'>Log In</div><div class='mb-12 px-4 text-gray-700 text-sm leading-none tracking-tighter'>Logging in as admin&nbsp;<a class='text-blue-700' href='#'>Not you?</a></div><div class='relative mb-6'><label class='absolute top-4 left-4 text-xs text-gray-700' for='password'>Password</label><input class='w-full border-y border-t-gray-300 border-b-gray-400 pt-8 px-4 pb-3.5 text-sm tracking-widest' id='password' name='password' type='password'></div><div class='flex mb-10 pl-4'>&nbsp;</div><div class='flex'><div class='hidden sm:block sm:w-60'>&nbsp;</div><button class='flex justify-between h-16 w-full bg-blue-600 px-4 pt-4 text-left text-sm text-white sm:w-60' type='submit'><span>Log in</span><svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 32 32' class='fill-current'><path d='M28 16 21 9 19.586 10.414 24.172 15 4 15 4 17 24.172 17 19.586 21.586 21 23 28 16z'></path></svg></button></div></form></div></body></html>"}]
        \r
        0\r
        \r
        """
