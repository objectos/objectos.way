...
public class HttpExchangeTest5ParseHeaders extends HttpExchangeTest {
  ...
  private final HttpHeaderName foo = HttpHeaderName.of("foo");
  ...
  public void name03() {
    test(
        ...
        Map.of(
            HttpHeaderName.of(tokenChars), "x"
        )
    );
  }
  ...
  public void name06() {
    ...
    try (HttpExchangeImpl http = HttpY.http(socket, 128, 256, Clock.systemDefaultZone(), Y.noteSink(), Long.MAX_VALUE)) {
  ...
  private void test(List<String> headers, Map<HttpHeaderName, Object> expected) {
    final String req;
    req = req(headers);
  ...
  private void test(Object request, Map<HttpHeaderName, Object> expected) {
    exec(test -> {
