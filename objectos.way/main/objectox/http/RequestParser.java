...
public final class RequestParser {
	...
	private BadRequestReason badRequest;
	...
	private HeaderName requestHeaderName;

	private Map<HeaderName, HeaderValue> requestHeaders;
	...
	private byte start() {
		...
		return toInputRead(_REQUEST_LINE);
	}
	...
	private byte toInputReadIfPossible(byte onRead, BadRequestReason onBufferFull) {
		if (bufferLimit < buffer.length) {
			return toInputRead(onRead);
		}

		if (bufferLimit == buffer.length) {
			return toBadRequest(onBufferFull);
		}
	...
	private byte requestLine() {
		...
		return switch (first) {
			...
			default -> toBadRequest(BadRequestReason.INVALID_METHOD);
		};
	}
	...
	private byte requestLineMethod() {
		...
		if (!bufferEquals(candidateBytes, candidateStart)) {
			...
			return toBadRequest(BadRequestReason.INVALID_METHOD);
		}
	...
	private byte requestLinePath() {
		...
		return toInputReadIfPossible(state, BadRequestReason.URI_TOO_LONG);
	}
	...
	private byte requestLineTarget() {
		...
		if (b != Bytes.SOLIDUS) {
			...
			return toBadRequest(BadRequestReason.TARGET_MUST_START_WITH_SOLIDUS);
		}
	...
	private byte requestLineVersion() {
		...
		if (buffer[index++] != 'H' ||
			...
			return toBadRequest(BadRequestReason.INVALID_PROTOCOL);
		}
	...
	private byte parseHeader() {
		...
		if (!bufferHasIndex(index)) {
			...
			return toInputReadIfPossible(state, BadRequestReason.INVALID_HEADER);
		}
		...
		if (first == Bytes.CR) {
			...
			if (second == Bytes.LF) {
				...
				return toRequestBodyIfNecessary();
			}
	...
	private byte toRequestBodyIfNecessary() {
		if (requestHeaders == null) {
			return _STOP;
		}
	...
	private byte requestBody() {
		...
		if (length < 0) {
			return toBadRequest(BadRequestReason.INVALID_CONTENT_LENGTH);
		}
	...
	private byte toBadRequest(BadRequestReason reason) {
		badRequest = reason;
