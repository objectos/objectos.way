 ...
 * @since 1
 */
final class TestCase06 extends StageGitCommand<ObjectId> {
  ...
  TestCase06(Directory target) {
    this.target = target;
  }

  public static void acceptGitServiceTest(GitServiceTest test) throws IOException {
    Directory target;
    target = test.testCase06Target = TmpDir.create();

    getDirectory().copyTo(target);

    TestCase06 impl;
    impl = new TestCase06(target);

    test.testCase06 = test.submit(impl);
  }
